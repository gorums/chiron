...
function viewMarks() {
  ...
  const books = Object.keys(S.bookmarks || {}).map(k => { const [mid, sec] = k.split(":"); return { mid, sec: +sec, ts: S.bookmarks[k] }; }).filter(b => byId(b.mid)).sort((a, b) => b.ts - a.ts);
  const f = markFilter;
  ...
  <div class="chips" style="margin-bottom:18px">
    ${[["all", "All"], ["open", "Open questions " + all.filter(m => m.status === "open").length], ["chats", "Conversations " + qCount], ["notes", "Notes " + all.filter(m => m.note).length], ["hl", "Highlights " + all.filter(m => !m.note && m.status !== "open").length], ["books", "Bookmarks " + books.length]]
  ...
  if (f === "all" || f === "hl" || f === "notes") {
    const list = all.filter(m => f === "notes" ? m.note : f === "hl" ? (!m.note && m.status !== "open") : m.status !== "open");
...
function markRow(m) {
  ...
  const kind = m.status === "open" ? "open question" : m.status === "answered" ? "answered" : m.note ? "note" : "highlight";
...
function exportNotes() {
  ...
  MODS.forEach(m => {
    const marks = marksOf(m.id), memo = (S.notes[m.id] || "").trim(), p = P(m.id);
    const books = m.sections.map((s, i) => S.bookmarks[m.id + ":" + i] ? s.h : null).filter(Boolean);
    ...
    elab.forEach(k => lines.push("**" + (m.assess.elaborate[k] || "Elaborate") + "**", "", p.elab[k], ""));
  });
  if (lines.length <= 2) { toast("Nothing to export yet"); return; }
