...
function confirmModal(title, body, okLabel, onOk, danger) {
  ...
  setTimeout(() => { const b = document.getElementById("modalok"); if (b) b.focus(); }, 20);
}
function promptModal(title, value, okLabel, onOk) {
  ...
  setTimeout(() => { const i = document.getElementById("modalin"); if (i) { i.focus(); i.select(); } }, 20);
}
function modalOk() { const cb = modalCb; modalCb = null; closeModal(); if (cb) cb(); }
...
function buildIndex() {
  ...
  idx.push({ t: "The plan", c: "Curriculum, how to study, the path to expert", h: "#/plan/curriculum" });
}
