...
function startSession(cards, mode) {
  session = { queue: shuffled(cards), total: cards.length, done: 0, flipped: false, mode: mode || "due", retired: 0 };
}
function cramAll() { startSession(allCards(), "due"); drawCard(); }
function drawCard() {
  const v = $("#view");
  if (!session || !session.queue.length) {
    const n = session ? session.done : 0, r = session ? session.retired : 0; session = null;
  ...
  v.innerHTML = `<div class="wrap">
    ...
    ${session.flipped ? `<div class="gradebar">
      ${[["Again", 0], ["Hard", 1], ["Good", 2], ["Easy", 3]].map(([l, g]) =>
...
function rate(g) {
  ...
  if (retired) { session.retired++; session.done++; toast("Retired — four clean recalls"); }
  else if (g === 0) session.queue.push(cur); else session.done++;
