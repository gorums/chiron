...
function viewHome() {
  ...
  <p class="sub" style="margin-bottom:22px">${STATE.biz ? esc(anchor.label) + ": <b>" + esc(STATE.biz) + '</b> · <a href="#/settings">change</a>' : `${esc(anchor.label)} is not set yet — <a href="#/settings">name it in Settings</a> so every exercise aims at something real.`}</p>`;
  ...
  if (weak.length || dropped.length) {
    h += `<div class="card" style="margin-bottom:18px;border-color:var(--warm)">
      ...
      ${dropped
        ...
        .map(
          m =>
            `<button class="btn sm" style="margin:0 8px 8px 0" onclick="go('#/m/${m.id}/2')">${m.id} · ${esc(m.short)} — slipped</button>`
        )
        .join("")}
    </div>`;
  }
