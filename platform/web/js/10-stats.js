...
function viewStats() {
  ...
  const c3 = qs.conf[3];
  h += `<p class="sub" style="margin-top:12px;font-size:12.5px">${c3[1] >= 5 && c3[0] / c3[1] < 0.8 ? "You are overconfident: when certain, you are right " + Math.round((c3[0] / c3[1]) * 100) + "% of the time. Slow down on the questions that feel obvious." : c3[1] >= 5 ? "Well calibrated. Your confidence is carrying real information." : "Answer more questions to see your calibration."}</p></div>`;
  ...
  for (let d = 0; d <= 14; d++) {
    ...
    h += `<div style="flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;height:100%" title="${n} card(s) in ${d} day(s)">
      <div style="width:100%;background:${d === 0 ? "var(--warm)" : "var(--accent)"};height:${n ? Math.max(4, (n / maxF) * 88) : 2}px;border-radius:3px;opacity:${n ? 1 : 0.25}"></div>
  ...
  per.forEach(x => {
    ...
    h += `<button class="mrow" style="padding:8px 10px;border-radius:8px;border-left:0" onclick="go('#/m/${x.m.id}')" title="${ms.name}">
      ...
      <span style="width:56px;text-align:right;font-size:12px;color:${x.pct == null ? "var(--muted)" : x.pct >= 0.8 ? "var(--ok)" : x.pct >= 0.6 ? "var(--warm)" : "var(--bad)"}">${x.pct == null ? "—" : Math.round(x.pct * 100) + "%"}</span></button>`;
  });
  h += `</div></div>`;
