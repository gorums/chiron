...
function renderSidebar() {
  ...
  <div class="navsec">
    ...
    <button class="navlink ${route.v === "review" ? "active" : ""}" onclick="go('#/review')"><span class="ico">${ICONS.review}</span>Review${due ? `<span class="pill" title="${mdue ? mdue + " of these are mistakes to fix" : ""}">${due}</span>` : ""}</button>
    <button class="navlink ${route.v === "check" ? "active" : ""}" onclick="go('#/check')"><span class="ico">◎</span>Checkpoints${checkOffers().length ? `<span class="pill">${checkOffers().length}</span>` : ""}</button>
  ...
  DATA.parts.forEach(p => {
    ...
    ms.forEach(m => {
      ...
      h += `<button class="mrow ${route.v === "m" && route.id === m.id ? "active" : ""}" onclick="go('#/m/${m.id}')" title="${ms_.name}">
        ...
        <span class="code">${m.id}</span><span class="t">${esc(m.short)}</span>${S.bookmarks && Object.keys(S.bookmarks).some(k => k.startsWith(m.id + ":")) ? `<span class="bm" title="Bookmarked">⚑</span>` : ""}</button>`;
    });
    h += `</div>`;
  });
  h += `<div class="navsec" style="border-top:1px solid var(--line);margin-top:10px">
    ...
    <button class="navlink ${route.v === "record" ? "active" : ""}" onclick="go('#/record')"><span class="ico">▣</span>Course record</button>
