...
Read `common/skills/course-author/references/pedagogy.md` before writing any prose. It is what separates this from a
...
- `notebooks` — only for a subject that is learned by running code (a language, data
  ...
  `common/skills/course-author/references/module-format.md`, "Notebooks"). Leave it out for every other subject.
...
**Read `common/skills/course-author/references/module-format.md` first — the format is a contract the build enforces.**
...
are in `common/skills/course-author/references/module-format.md` under "Figures"; the build enforces them. One or two
per module where they earn their place; none where they do not. Where it goes decides
whether it is remembered: `common/skills/course-author/references/pedagogy.md`, "Figures and notebooks: where they
...
`[caption](notebooks/<ID>-<n>.ipynb)`. The rules are in `common/skills/course-author/references/module-format.md`
under "Notebooks", and where one goes (the exercise, a live worked example, a cell that
fails) in `common/skills/course-author/references/pedagogy.md`. A course without that field may not refer to one,
...
- `templates/*.md` — fill-in worksheets the reader reuses. First line `# Title`, then a
  ...
  blocks become real inputs on the page (see `common/skills/course-author/references/module-format.md`). These are the
  course's lasting artefact.

## Phase 6 — study data

**Read `common/skills/course-author/references/data-schemas.md`.** Two JSON families, and one invariant that the build
